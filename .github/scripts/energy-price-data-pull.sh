...
QUERY="dataset/elspotprices?limit=${LIMIT}&sort=HourUTC%20desc&columns=HourUTC,PriceArea,SpotPriceEUR&start=$(date -d "@$FROMDATE" +"%Y-%m-%dT%H:%M")&end=$(date -d "@$ENDDATE" +"%Y-%m-%dT%H:%M")&timezone=UTC"
...
while true; do
    ...
    TRANSFORMATION='.records |= map(. as $item | {euro: $item.SpotPriceEUR, timestamp: $item.HourUTC}) | .records[].timestamp |= (split("+")[0] + "Z"|fromdateiso8601) | .records'
