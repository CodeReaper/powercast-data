...
for DESTINATION in "$FOLDER"/????/??/??; do
  ...
  find "${DESTINATION}" -type f \( -name "${AREA}.json" -o -name "${AREA}-*.json" \) -print0 | xargs -0 jq -s '. | unique_by(.timestamp) | sort_by(.timestamp)' > /tmp/$$.json && mv /tmp/$$.json "${DESTINATION}/${AREA}.json"
