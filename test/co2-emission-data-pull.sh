...
set +e
unset WGET_OVERRIDE
sh src/co2-emission-data-pull.sh 1658318400 1658361600 > /dev/null
[ $? -ne 0 ] || exit 1
...
sh src/co2-emission-data-pull.sh 1658318400 1658361600 > /tmp/t/result
...
now=$(($(date +"%s")+86400))
tomorrow=$((now+86400))
sh src/co2-emission-data-pull.sh $now $tomorrow > /dev/null
[ $? -ne 0 ] || exit 1
